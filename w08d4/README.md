...
### To Do
- [ ] Review ES6 Classes (show the 'must call super constructor' error)
- [ ] Intro to Class-based Components (render method, this.props, )
- [ ] Passing Props
- [ ] Changes to State (initialize state in constructor, which could induce 'must call super' error)
- [ ] Handling Events (helper function can be defined inside the class, no spread operator, this.setState merges values or takes a callback with prev arg. onClick={this.clickHandler} will not have this defined. fix this with bind )
- [ ] Lifecycle Methods (put a setInterval into the component)
